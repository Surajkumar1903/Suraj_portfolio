...
window.addEventListener("scroll",()=>{
  ...
  document.querySelectorAll(".hero-blob").forEach((o,i)=>{o.style.transform=`translateY(${y*(0.06+i*0.03)}px)`});
});
...
(function initHeroParallax(){
  ...
  hero.addEventListener('mousemove', e => {
    ...
    wrap.style.transform = `translate(${mx * -14}px, ${my * -10}px)`;
  });
...
// ── Smooth 3D tilt on photo ──────────────────────────────────────────────────
(function initHeroTilt(){
  const frame = document.getElementById('heroCard');
  if(!frame) return;
  frame.addEventListener('mousemove', e => {
    ...
    frame.style.transform = `perspective(700px) rotateX(${rx}deg) rotateY(${ry}deg) scale(1.03)`;
  });
  frame.addEventListener('mouseleave', () => {
    frame.style.transform = 'perspective(700px) rotateX(0) rotateY(0) scale(1)';
  });
})();
